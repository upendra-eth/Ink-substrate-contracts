...
pub enum Error {
    ...
    /// Onwer does not have enough token value
    NotEnoughValue,
}
...
mod erc1155 {
    ...
    pub struct Contract {
        ...
        token_id_to_uri: BTreeMap<TokenId, Vec<u8>>,
        token_uri : Vec<u8>,
    }
    ...
    impl Contract {
        ...
        /// to mint tokens.
        #[ink(message)]
        pub fn mint(&mut self,value: Balance, uri: Vec<u8>) -> Result<()> {
           ...
           self.token_id_nonce += 1;
            ...
            self.token_id_to_uri.insert(self.token_id_nonce, uri.clone());
        ...
        #[ink(message)]
        pub fn burn(&mut self, token_id: TokenId,value: Balance) -> Result<()> {
            ensure!(token_id <= self.token_id_nonce, Error::UnexistentToken);
            ensure!(value <= *self.balances.get(&(self.env().caller(), token_id)).unwrap_or(&0), Error::NotEnoughValue);

            let caller = self.env().caller();

            let left_value = self.balances.get(&(caller, token_id)).unwrap_or(&0) - value;
            
            self.balances.insert((caller, token_id),left_value);
