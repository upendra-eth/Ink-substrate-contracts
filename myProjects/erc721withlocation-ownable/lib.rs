...
#[ink::contract]
mod erc721withlocation_ownable {
    ...
    #[ink(storage)]
    #[derive(Default)]
    pub struct Erc721withlocationOwnableRef {
        ...
        id_to_uri: StorageHashMap<TokenId, Vec<u8>>,
        /// Mapping from token to location
        id_to_location: StorageHashMap<TokenId, Vec<u8>>,
    ...
    /// Event emitted when native token transfer from contract occurs.
    #[ink(event)]
    ...
    /// Event emitted when a mint_token occurs.
    #[ink(event)]
    pub struct Mint {
        #[ink(topic)]
        to: Option<AccountId>,
    ...
    /// Event emitted when a burn token occurs.
    #[ink(event)]
    pub struct Burn {
        #[ink(topic)]
        burn_from: Option<AccountId>,
        #[ink(topic)]
        id: TokenId,
    }
    ...
    impl Erc721withlocationOwnableRef {
        /// Creates a new ERC-721 token contract.
        #[ink(constructor)]
        pub fn new(_admin: AccountId, _manager: AccountId) -> Self {
            Self {
        ...
        #[ink(message)]
        pub fn get_uri(&self, id: TokenId) -> Option<Vec<u8>> {
            self.id_to_uri.get(&id).cloned()
        }

        #[ink(message)]
        pub fn get_location(&self, id: TokenId) -> Option<Vec<u8>> {
            self.id_to_location.get(&id).cloned()
        }
        ...
        pub fn transfer_from_caller(
            ...
            self.id_to_location.insert(id, new_location.clone());
        ...
        /// Transfers the token from the caller to the given destination with restriction
        /// Only current manager can access this function
        ...
        /// Transfers `value` amount of tokens to the caller(only manager can call).
        ///
        /// # Errors
        ///
        ...
        #[ink(message)]
        pub fn give_me(&mut self, _transfer_to: AccountId, value: Balance) -> Result<(), Error> {
            let caller = self.env().caller();
            if caller != self.manager {
                return Err(Error::UnauthorizedCaller);
            };
            ...
            if self.env().transfer(_transfer_to, value).is_err() {
                panic!(
            ...
            self.env().emit_event(TransferBalance {
                to: Some(_transfer_to),
                value: Some(value),
            });
            Ok(())
        }

        /// Minting a new token at creators address and transfering it to buyers account
        /// Only current manager can access this function
        #[ink(message)]
        pub fn redeem(
            &mut self,
            ...
            self.mint(_creator, uri, location.clone())?;
            self.transfer(_creator, _owner, self.nft_count, location)?;

            Ok(())
        }

        /// Creates a new token to caller address
        #[ink(message)]
        pub fn mint_nft(&mut self, uri: Vec<u8>, location: Vec<u8>) -> Result<(), Error> {
            let caller = self.env().caller();
            self.mint(caller, uri, location)?;
            Ok(())
        }
        ...
        fn mint(
            ...
            self.nft_count += 1;
            self.add_token_to(&_creator, self.nft_count)?;
            self.id_to_uri.insert(self.nft_count, uri);
            self.id_to_location.insert(self.nft_count, location.clone());
        ...
        pub fn burn(&mut self, id: TokenId) -> Result<(), Error> {
            ...
            let occupied1 = match self.id_to_uri.entry(id) {
                Entry::Vacant(_) => return Err(Error::TokenNotFound),
                Entry::Occupied(occupied1) => occupied1,
            };

            let occupied2 = match self.id_to_location.entry(id) {
                Entry::Vacant(_) => return Err(Error::TokenNotFound),
                Entry::Occupied(occupied2) => occupied2,
            };
            ...
            occupied1.remove_entry();
            occupied2.remove_entry();
            self.env().emit_event(Burn {
                burn_from: Some(caller),
                id,
            });
            Ok(())
        }
    ...
    mod tests {
        ...
        fn mint_works() {
            ...
            let mut erc721withlocation_ownable = Erc721withlocationOwnableRef::new();
            // Token 1 does not exists.
            assert_eq!(erc721withlocation_ownable.owner_of(1), None);
            // Alice does not owns tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.alice), 0);
            // Create token Id 1.
            assert_eq!(erc721withlocation_ownable.mint(1), Ok(()));
            // Alice owns 1 token.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.alice), 1);
        }
        ...
        fn mint_existing_should_fail() {
            ...
            assert_eq!(erc721withlocation_ownable.owner_of(1), Some(accounts.alice));
            // Cannot create  token Id if it exists.
            // Bob cannot own token Id 1.
            assert_eq!(erc721withlocation_ownable.mint(1), Err(Error::TokenExists));
        }
        ...
        fn transfer_works() {
            ...
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.bob), 0);
            // The first Transfer event takes place
            assert_eq!(1, ink_env::test::recorded_events().count());
            // Alice transfers token 1 to Bob
            assert_eq!(erc721withlocation_ownable.transfer(accounts.bob, 1), Ok(()));
            // The second Transfer event takes place
            assert_eq!(2, ink_env::test::recorded_events().count());
            // Bob owns token 1
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.bob), 1);
        }
        ...
        fn invalid_transfer_should_fail() {
            ...
            assert_eq!(erc721withlocation_ownable.transfer(accounts.bob, 2), Err(Error::TokenNotFound));
            // Token Id 2 does not exists.
            assert_eq!(erc721withlocation_ownable.owner_of(2), None);
            // Create token Id 2.
            assert_eq!(erc721withlocation_ownable.mint(2), Ok(()));
            // Alice owns 1 token.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.alice), 1);
            // Token Id 2 is owned by Alice.
            assert_eq!(erc721withlocation_ownable.owner_of(2), Some(accounts.alice));
            ...
            assert_eq!(erc721withlocation_ownable.transfer(accounts.eve, 2), Err(Error::NotApproved));
        }
        ...
        fn approved_transfer_works() {
            ...
            assert_eq!(erc721withlocation_ownable.approve(accounts.bob, 1), Ok(()));
            // Get contract address.
            let callee = ink_env::account_id::<ink_env::DefaultEnvironment>();
            // Create call
            let mut data = ink_env::test::CallData::new(ink_env::call::Selector::new([0x00; 4])); // balance_of
            ...
            assert_eq!(
                erc721withlocation_ownable.transfer_from(accounts.alice, accounts.eve, 1),
                Ok(())
            );
            // TokenId 3 is owned by Eve.
            assert_eq!(erc721withlocation_ownable.owner_of(1), Some(accounts.eve));
            // Alice does not owns tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.alice), 0);
            // Bob does not owns tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.bob), 0);
            // Eve owns 1 token.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.eve), 1);
        }
        ...
        fn approved_for_all_works() {
            ...
            assert_eq!(erc721withlocation_ownable.mint(2), Ok(()));
            // Alice owns 2 tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.alice), 2);
            // Approve token Id 1 transfer for Bob on behalf of Alice.
            assert_eq!(erc721withlocation_ownable.set_approval_for_all(accounts.bob, true), Ok(()));
            // Bob is an approved operator for Alice
            assert!(erc721withlocation_ownable.is_approved_for_all(accounts.alice, accounts.bob));
            // Get contract address.
            let callee = ink_env::account_id::<ink_env::DefaultEnvironment>();
            // Create call
            let mut data = ink_env::test::CallData::new(ink_env::call::Selector::new([0x00; 4])); // balance_of
            ...
            assert_eq!(
                erc721withlocation_ownable.transfer_from(accounts.alice, accounts.eve, 2),
                Ok(())
            );
            // Bob does not owns tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.bob), 0);
            // Eve owns 2 tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.eve), 2);
            ...
            assert_eq!(erc721withlocation_ownable.set_approval_for_all(accounts.bob, false), Ok(()));
        ...
        fn not_approved_transfer_should_fail() {
            ...
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.eve), 0);
            // Get contract address.
            let callee = ink_env::account_id::<ink_env::DefaultEnvironment>();
            // Create call
            let mut data = ink_env::test::CallData::new(ink_env::call::Selector::new([0x00; 4])); // balance_of
            ...
            assert_eq!(
                erc721withlocation_ownable.transfer_from(accounts.alice, accounts.frank, 1),
        ...
        fn burn_works() {
            ...
            assert_eq!(erc721withlocation_ownable.owner_of(1), Some(accounts.alice));
            // Destroy token Id 1.
            assert_eq!(erc721withlocation_ownable.burn(1), Ok(()));
            // Alice does not owns tokens.
            assert_eq!(erc721withlocation_ownable.balance_of(accounts.alice), 0);
        ...
        fn burn_fails_token_not_found() {
            ...
            assert_eq!(erc721withlocation_ownable.burn(1), Err(Error::TokenNotFound));
        }
        ...
        fn burn_fails_not_owner() {
            ...
            assert_eq!(erc721withlocation_ownable.burn(1), Err(Error::NotOwner));
        }
